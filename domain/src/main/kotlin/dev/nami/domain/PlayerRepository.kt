...
/** Откуда сейчас читаются байты текущего трека (Дизайн.md §4.13).
 *
 * Скачанный с сервера трек - это [LOCAL]: файл уже лежит на устройстве и играет с накопителя,
 * сеть для него больше не нужна. Отдельного состояния «скачан» здесь нет намеренно; то, что
 * трек вдобавок есть на сервере, показывает отдельный признак
 * [PlayerRepository.currentTrackOnServer], потому что это независимый вопрос. */
enum class TrackPlaybackSource {
    /** Файл на устройстве: своя библиотека или ранее скачанный с сервера трек. */
    LOCAL,
...
interface PlayerRepository {
    ...
    /** Есть ли текущий трек на сервере Nami. `null` - ответа нет: сервер не подключён или
     * ещё не ответил, и «нет на сервере» в этом случае было бы враньём. Вопрос независим от
     * [playbackSource]: трек может играть с устройства и при этом лежать на сервере. */
    val currentTrackOnServer: StateFlow<Boolean?>
