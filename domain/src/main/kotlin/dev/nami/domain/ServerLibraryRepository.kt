...
interface ServerLibraryRepository {
    ...
    /** Удаляет с сервера треки, сопоставленные по метаданным. Нужен для локальных треков: их
     * серверный id заранее неизвестен, у зеркал он зашит в id строки, а у обычного файла нет.
     * Возвращает id фактически удалённых - трека может не быть на сервере вовсе. */
    suspend fun deleteMatchingTracks(tracks: List<ServerTrackMeta>): List<Long> = emptyList()
