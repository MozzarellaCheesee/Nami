...
interface SettingsRepository {
    ...
    /** Экран, который открывается на холодном старте приложения - тот же [BottomTab], что и
     * вкладки таб-бара (переиспользуем набор, не заводим отдельный enum под одно то же самое).
     * Значение вкладки, которую пользователь выключил в таб-баре, тоже допустимо - экран
     * остаётся доступным напрямую даже без своей кнопки внизу. */
    val defaultStartScreen: StateFlow<BottomTab>
    fun setDefaultStartScreen(tab: BottomTab)
