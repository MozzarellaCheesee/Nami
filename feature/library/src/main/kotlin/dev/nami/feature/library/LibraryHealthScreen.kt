...
private fun HealthCategory(
    ...
    Column(modifier = Modifier.padding(top = 8.dp).background(NamiColors.Ink800, RoundedCornerShape(NamiRadius.Button)).animateContentSize().clickable(enabled = count > 0) { expanded = !expanded }) {
...
private fun YearlessAlbumsCategory(albums: List<HealthAlbumRef>, onSave: (AlbumId, Int) -> Unit) {
    var expanded by remember { mutableStateOf(false) }
    Column(modifier = Modifier.padding(top = 8.dp).background(NamiColors.Ink800, RoundedCornerShape(NamiRadius.Button)).animateContentSize().clickable(enabled = albums.isNotEmpty()) { expanded = !expanded }) {
        Row(modifier = Modifier.fillMaxWidth().padding(16.dp), verticalAlignment = Alignment.CenterVertically) {
            Icon(
...
private fun InconsistentArtistNamesCategory(groups: List<List<HealthArtistRef>>, onMerge: (List<dev.nami.core.model.ArtistId>, String) -> Unit) {
    var expanded by remember { mutableStateOf(false) }
    Column(modifier = Modifier.padding(top = 8.dp).background(NamiColors.Ink800, RoundedCornerShape(NamiRadius.Button)).animateContentSize().clickable(enabled = groups.isNotEmpty()) { expanded = !expanded }) {
        Row(modifier = Modifier.fillMaxWidth().padding(16.dp), verticalAlignment = Alignment.CenterVertically) {
            Icon(
