            ...
            if (showMenu) {
                ContextActionSheet(
                    ...
                    actions = listOfNotNull(
                        onLikeTrack?.let { ContextAction("В любимые", Icons.Outlined.FavoriteBorder, onClick = it) },
                        onAddToPlaylist?.let { ContextAction("В плейлист", Icons.Outlined.LibraryAdd, onClick = it) },
                        onAddToQueue?.let { ContextAction("В очередь", Icons.Outlined.PlaylistAdd, onClick = it) },
                        onRename?.let { ContextAction("Переименовать", Icons.Outlined.Edit, onClick = it) },
                        onEditNote?.let { ContextAction("Заметка", Icons.Outlined.Notes, onClick = it) },
                        onEditTags?.let { ContextAction("Редактировать теги", Icons.Outlined.Label, onClick = it) },
