...
```jsonc
{
  ...
  "to": "2026-09-30",
  "available_from": "2026-09-28",  // когда доступны (необязательно, см. ниже)
  "available_until": "2026-09-30",
  "min_plays": 10,                 // меньше прослушиваний - итоги не показываются
  "auto_open": true,               // открыть у всех само, один раз при запуске (необязательно)
  "theme": { ... },                // стиль по умолчанию для всех экранов
  "screens": [ ... ]
}
```

Если `available_from`/`available_until` не указаны: итоги за целый месяц доступны в последние 3 дня месяца, итоги за целый год — с 25 декабря по 7 января. Пока окно идёт, карточка итогов видна на главной и в «Статистике», итоги можно открывать снова. С `auto_open` они ещё и открываются сами один раз при запуске.
