...
сколько осталось. Значения отдаются в `GET /api/tracks/{id}` (`replaygain_track_gain`,
`replaygain_track_peak`, `r128_loudness`; `fingerprint` - только в этой ручке, в списке
треков он был бы лишними килобайтами на строку).

**BPM и тональность пока не считаются** - готового лёгкого способа в Rust нет, а тянуть
ради них тяжёлую зависимость против бюджета «1 ядро / 512 МБ». Появятся вместе с DJ-режимом
и офлайн-радио, которым они и нужны.
...
`stream`, `download`, `scrobble` (заглушка - историю ведёт клиент в своём состоянии).
Всё остальное отвечает «не поддерживается»: правдоподобная пустышка, на которой клиент
построит своё UI, хуже честной ошибки. Редактирование плейлистов через Subsonic-протокол
не предусмотрено - плейлисты отдаются на чтение.
...
- Веб-клиент и установка одной командой - отдельные этапы.
- OpenSubsonic реализован подмножеством (см. раздел выше): обложки, звёзды, радио,
  подкасты и редактирование плейлистов через `/rest/` не поддерживаются.
