...
как есть, без ресайза), `scrobble` (заглушка - историю ведёт клиент в своём состоянии).
...
- OpenSubsonic реализован подмножеством (см. раздел выше): обложки отдаются как есть
  (`getCoverArt`), но без ресайза; звёзды, радио, подкасты и редактирование плейлистов
