...
оценки - пишутся в общий `state`, см. ниже), `scrobble` (заглушка - историю ведёт клиент в
своём состоянии). Всё остальное отвечает «не поддерживается»: правдоподобная пустышка, на
